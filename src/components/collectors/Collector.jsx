...
export default function CollectorColumn({ type = "", item}) {
    return (
        <div key={`user_${item.id}`} className={(item.id == 2 || item.id == 5 || item.id == 8 || item.id == 11) ? classNames(styles.collector_container, styles.container_light) : classNames(styles.collector_container)}>
